...
async function main() {
  ...
  const gitpoap4 = await GitPOAPFactory.createGitPOAP(2020, 19375, repo1.id, 'secret4', 'hash-browns-4');
  // For GitPOAP BE Repo ~ Using GitPOAP Strategy Meeting POAP for now ~ eventID: 29009
  const gitpoap5 = await GitPOAPFactory.createGitPOAP(2020, 29009, repo2.id, 'secret5', 'hash-browns-5'); // For GitPOAP BE Repo
