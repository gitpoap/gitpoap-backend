...
async function main() {
  ...
  /* Create GitPOAPs */
  const gitpoap1 = await GitPOAPFactory.createGitPOAP(event1.name, event1.image_url, event1.description, 2022, 1, repo34Project.id, '012345', GitPOAPStatus.APPROVED);
  const gitpoap2 = await GitPOAPFactory.createGitPOAP(event2.name, event2.image_url, event2.description, 2024, 2, repo7Project.id, '123456', GitPOAPStatus.APPROVED);
  const gitpoap3 = await GitPOAPFactory.createGitPOAP(event3.name, event3.image_url, event3.description, 2015, 3, repo568Project.id, '234567', GitPOAPStatus.APPROVED);
  // For GitPOAP FE Repo ~ Using generic GitPOAP related POAP for now ~ eventID: 19375
  const gitpoap4 = await GitPOAPFactory.createGitPOAP(event19375.name, event19375.image_url, event19375.description, 2020, 19375, frontendProject.id, '345678', GitPOAPStatus.APPROVED);
  // For GitPOAP BE Repo ~ Using GitPOAP Strategy Meeting POAP for now ~ eventID: 29009
  const gitpoap5 = await GitPOAPFactory.createGitPOAP(event29009.name, event29009.image_url, event29009.description, 2020, 29009, backendProject.id, '456789', GitPOAPStatus.APPROVED);
  // For burz/dopex repo ~ eventID: 34634
  const gitpoap6 = await GitPOAPFactory.createGitPOAP(event34634.name, event34634.image_url, event34634.description, 2022, 34634, dopexProject.id, '304374');

  // For the gitpoaps created for the bug bash -~- March 2022
  const gitpoap7 = await GitPOAPFactory.createGitPOAP(event36568.name, event36568.image_url, event36568.description, 2022, 36568, frontendProject.id, '669250', GitPOAPStatus.APPROVED);
  const gitpoap8 = await GitPOAPFactory.createGitPOAP(event36569.name, event36569.image_url, event36569.description, 2022, 36569, frontendProject.id, '102513', GitPOAPStatus.APPROVED, false, 2, 2);
  const gitpoap9 = await GitPOAPFactory.createGitPOAP(event36570.name, event36570.image_url, event36570.description, 2022, 36570, backendProject.id, '929862', GitPOAPStatus.APPROVED);
  const gitpoap10 = await GitPOAPFactory.createGitPOAP(event36571.name, event36571.image_url, event36571.description, 2022, 36571, backendProject.id, '252134', GitPOAPStatus.APPROVED, false, 2, 2);
  const gitpoap11 = await GitPOAPFactory.createGitPOAP(event36572.name, event36572.image_url, event36572.description, 2022, 36572, backendProject.id, '613740', GitPOAPStatus.APPROVED, false, 3, 3);
  // Not the real POAP secret!
  const gitpoap12 = await GitPOAPFactory.createGitPOAP(event37428.name, event37428.image_url, event37428.description, 2022, 37428, wagyuInstallerProject.id, '324324', GitPOAPStatus.UNAPPROVED, true);
  const gitpoap13 = await GitPOAPFactory.createGitPOAP(event37429.name, event37429.image_url, event37429.description, 2022, 37429, wagyuInstallerProject.id, '324325', GitPOAPStatus.UNAPPROVED, true, 2, 2);
  const gitpoap14 = await GitPOAPFactory.createGitPOAP(event37430.name, event37430.image_url, event37430.description, 2022, 37430, wagyuInstallerProject.id, '324326', GitPOAPStatus.UNAPPROVED, true, 3, 3);

  // For gitpoap-bot-test-repo (uses random POAP IDs)
  const gitpoap15 = await GitPOAPFactory.createGitPOAP(event36573.name, event36573.image_url, event36573.description, 2022, 36573, botTestProject.id, '123456', GitPOAPStatus.APPROVED, true);
  const gitpoap16 = await GitPOAPFactory.createGitPOAP(event36574.name, event36574.image_url, event36574.description, 2022, 36574, botTestProject.id, '123456', GitPOAPStatus.APPROVED, true, 2, 2);
