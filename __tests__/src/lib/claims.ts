...
test('createNewClaimsForRepoPR: Does nothing when threshold is not met', async () => {
...
test('createNewClaimsForRepoPR: Creates claim when threshold is met', async () => {
  contextMock.prisma.$queryRaw.mockResolvedValue([{ count: 1 }]);
...
test('createNewClaimsForRepoPR: Creates multiple claims when thresholds are met', async () => {
