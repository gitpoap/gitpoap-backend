...
profilesRouter.post('/', async function (req, res) {
  logger.debug(`POST /profiles: Body: ${req.body}`);

  const schemaResult = UpdateProfileSchema.safeParse(req.body);

  if (!schemaResult.success) {
    logger.warn(
      `POST /profiles: Missing/invalid body fields in request: ${schemaResult.error.issues}`,
    );
    return res.status(400).send({ issues: schemaResult.error.issues });
  }

  logger.info(`POST /profiles: Request to update profile for address: ${req.body.address}`);

  const resolvedAddress = await resolveENS(context.provider, req.body.address);
  if (resolvedAddress === null) {
    logger.warn('POST /profiles: Request address is invalid');
  ...
  if (recoveredAddress !== resolvedAddress) {
    logger.warn('POST /profiles: Request signature is invalid');
  ...
  try {
    ...
    logger.warn(`POST /profiles: No profile for address ${req.body.address}`);
    return res.status(404).send({ msg: `No profile found for address: ${req.body.address}` });
  }

  logger.debug(
    `POST /profiles: Completed request to update profile for address: ${req.body.address}`,
  );
